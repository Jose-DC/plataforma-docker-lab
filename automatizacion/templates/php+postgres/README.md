# Plantilla PHP + PostgreSQL

Entorno de desarrollo para una aplicación PHP mínima con persistencia en
PostgreSQL. El ejemplo incluido permite validar el servidor web y la conexión
...
- `app`: aplicación PHP construida desde `app/Dockerfile`.
...
```bash
make crear nombre=demo_php_postgres stack=php+postgres lang=8.2 dbv=16
```
...
La aplicación incluida es una prueba técnica. No incorpora autenticación,
